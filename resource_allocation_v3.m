function x0 = resource_allocation_v3(x0,num_UE,num_CH,num_TP,path_loss,noise,average_rate,power_bound,B_max,BS_Node,power_max,r_type)
% r_type
% 0: TP-based selection method (proposed method)
% 1: PSO (same as 0 in this function)
% 2: Greedy method 
% 3: Whole (RRH select UE in the same manner as BS)
% 4: Complex RRH (Branch and Bound for RRH)
% 5: Complex BS (more candidate for each power level)
% 6: Greedy with single round (not in this function)
% 7: Discrete selection (not in this function)
% 9: Randomized algorithm (same as 2 in this function)
r_type_ini = r_type;
...
%         Power(:,j) = power_bound(1,:,j)';
        power_bound(1,:,j) = Power(:,j)';
    else
        Power(:,j) = x0(num_UE+1,:,j)';
    end
...
for count_temp = 1:count_temp_size
    ...
    for b = TP_priority
        ...
        elseif r_type==2
            for j = consider_CH
                candidate_UE = candidate_set(b,1:find(candidate_set(b,:,j)==0,1,'first')-1,j);
                if ~isempty(candidate_UE)
                    [~,serving_UE_index(b,j)] = max(log2(1+power_bound(1,b,j)*path_loss(candidate_UE,b,j)./I(candidate_UE,j))./average_rate(candidate_UE));
                    serving_UE_index(b,j) = candidate_UE(serving_UE_index(b,j));
                    S(serving_UE_index(b,j),j) = S(serving_UE_index(b,j),j) + power_bound(1,b,j)*path_loss(serving_UE_index(b,j),b,j);
                    I(:,j) = I(:,j) + power_bound(1,b,j)*path_loss(:,b,j);
                    I(serving_UE_index(b,j),j) = I(serving_UE_index(b,j),j) - power_bound(1,b,j)*path_loss(serving_UE_index(b,j),b,j);
                end
            end
        elseif r_type==5
            [serving_UE_index(b,consider_CH) Power(b,consider_CH) S(:,consider_CH) I(:,consider_CH)] = UE_Selection_BS_v4_complex(S(:,consider_CH),I(:,consider_CH),path_loss(:,:,consider_CH),...
                candidate_set(b,:,consider_CH),num_TP,length(consider_CH),power_upper,average_rate,limit_power(consider_CH),Power(b,consider_CH),b,temperature,ratio^2);
        end
    end
    ...
    for b = TP_priority
        ...
        elseif r_type==3
            [serving_UE_index(b,consider_CH) Power(b,consider_CH) S(:,consider_CH) I(:,consider_CH)] = UE_Selection_BS_v4(S(:,consider_CH),I(:,consider_CH),path_loss(:,:,consider_CH),...
                candidate_set(b,:,consider_CH),num_TP,length(consider_CH),power_upper,average_rate,power_upper,Power(b,consider_CH),b,temperature,ratio^2);
        end

    end
    if r_type==4
        [serving_UE_index(RRH_Node,consider_CH) S(:,consider_CH) I(:,consider_CH)] = ...
            UE_Selection_RRH_ctd(S(:,consider_CH),I(:,consider_CH),path_loss(:,:,consider_CH),candidate_set(:,:,consider_CH),...
            length(consider_CH),num_TP,BS_Node,serving_UE_index(BS_Node,consider_CH),Power(:,consider_CH),average_rate,noise);
    end
