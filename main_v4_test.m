function tt = main_v4_center(c_str,m_str,b_str)
%%
% Input
% c_str: data index
...
for i=1:sample
    ...
    for t=1:T
        ...
        while round < round_max
            ...
            % x0 = resource_allocation_v3(x0,num_UE,num_CH,num_TP,path_loss,noise,average_rate,power_bound,B_max,1,power_max,r_type);
            if t<=0
                x0 = TP_based_selection(x0,num_UE,num_CH,num_TP,path_loss,noise,average_rate,power_bound,B_max,1,power_max);
            else
                x0 = ResourceAllocation_CCCP_v1(x0,num_UE,num_CH,num_TP,path_loss,noise,average_rate,power_bound,mu_t,);
            end
            ...
            % [~,x0(num_UE+1,:,:)] = bound_update_iterative(x0,power_max,path_loss,noise,average_rate,num_UE,num_TP,num_CH,0,bound_mode,iteration_max,p_type);
            if t<=0
                [~,x0(num_UE+1,:,:)] = two_layer_power_control(x0,power_max,path_loss,noise,average_rate,num_UE,num_TP,num_CH,iteration_max);
            else
                [~,x0(num_UE+1,:,:)] = PowerControl_CCCP_v0(x0,power_max,path_loss,noise,average_rate,num_UE,num_TP,num_CH,iteration_max);
            end
            % fprintf('t = %d, r = %d\n',t,round);
            % pwr = squeeze(x0(end,:,:))
...
resultpath = './fmincon_ra_1';
save(fullfile(resultpath,['result_UE_' m_str '_CH_' b_str '_round_' int2str(cc/sample+1)]),'ave_fairness','ave_capacity','ratio_performance',...
