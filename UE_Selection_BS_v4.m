function [serving_UE_index Power S I] = UE_Selection_BS_v4(S,I,path_loss,candidate_UE_all,num_TP,num_CH,power_bound,average_rate,power_limit,Power,BS_node,temperature,ratio)
% review down
...
for j = 1:num_CH
    ...
    if ~isempty(candidate_UE)
        if power_bound(j) > 1e-6
            ...
            prob_selecting = exp((ratio*(object_value-max_u0)/max_u0+(1-ratio)*bias)/temperature*10);
